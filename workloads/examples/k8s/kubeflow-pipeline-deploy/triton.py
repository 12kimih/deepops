...
Example Triton Inference Server Models: https://github.com/triton-inference-server/server
Example Triton Inference Server Client: https://github.com/triton-inference-server/client
Bugs:
  Cannot dynamically assign GPU counts: https://github.com/kubeflow/pipelines/issues/1956

# Manual run example:
docker run --gpus all --rm --shm-size=1g --ulimit memlock=-1 --ulimit stack=67108864 -p8000:8000 -p8001:8001 -p8002:8002 -v/raid/shared/results/model_repository/:/model_repository nvcr.io/nvidia/tritonserver:21.02-py3 tritonserver --model-repository=/model_repository
docker run -it --rm --net=host nvcr.io/nvidia/tritonserver:21.02-py3-sdk /workspace/install/bin/image_client -m densenet_onnx -c 3 -s INCEPTION /workspace/images/mug.jpg
'''
import triton_ops
...
def triton_pipeline(skip_examples):
    ...
    # Set order so that volumes are created, then examples downloaded, then service started
