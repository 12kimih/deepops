...
if [ "${HELM_MINIMUM_VERSION}" != "${helm_min_installed}" ]; then
    ...
    HELM_INSTALL_DIR=${HELM_INSTALL_DIR} DESIRED_VERSION=v3.17.1 /var/tmp/get_helm.sh # Keep near helm_version in the bundled Kubespray (roles/kubespray_defaults)
fi
